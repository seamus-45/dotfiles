...
if [ $# -lt 1 ]; then
  ...
  while echo -ne ${ps}; read URI; do
    [[ -z ${URI} ]] && exit 0
    mplayer -geometry 500x300+2850+680 -really-quiet -prefer-ipv4 $(quvi dump -p rfc2483 "${URI}" | grep -v \#) &
    # -ontop
    ...
    mplayer -geometry 500x300+2850+680 -really-quiet -prefer-ipv4 $(quvi dump -p rfc2483 "${1}" | grep -v \#) &
    # -ontop
    #until WID=$(wmctrl -l | grep MPlayer); do sleep 1; done
    #echo "Found mplayer window $(echo $WID | awk '{print $1}'). Make it sticky."
    #wmctrl -r mplayer -b add,sticky
